...
<button
	...
	class="w-full rounded-xl border border-gray-100 bg-white p-4 text-left shadow-sm transition-shadow hover:shadow-md"
>
		...
		<div class="min-w-0 flex-1">
			<div class="flex items-start justify-between">
				...
				<div class="text-right">
					<p class="font-semibold text-gray-900">NT$ {formatTwd(totalTwd)}</p>
					<p class="text-xs text-gray-400">{lastDate} 更新</p>
				</div>
			</div>
		</div>
	</div>
	...
	{#if expanded && entries.length > 0}
		<div class="mt-3 ml-4 space-y-1 border-t pt-3">
				...
				<div class="flex justify-between text-sm">
					...
					<span class="text-gray-700">
						{#if entry.original_currency !== 'TWD'}
							<span class="text-xs text-gray-400">{formatOriginal(entry)}</span>
						{/if}
	...
	<div class="mt-2 ml-4 text-center">
		<span class="text-xs text-gray-300">{expanded ? '▲' : '▼'}</span>
	</div>
</button>
